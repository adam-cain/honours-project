...
export default function BackButton({ backButtonConfig ,isNavCollapsed, hideText }: { backButtonConfig: BackButtonConfig,isNavCollapsed: boolean, hideText: boolean }) {
    ...
    const active = true
    return (
        <Link className={` border h-10 flex items-center gap-3 rounded-lg px-3 py-2 text-gray-500 transition-all hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-50`} href={backButtonConfig.href}>
        <div><LucideArrowLeft className="h-4 w-4"/></div>
